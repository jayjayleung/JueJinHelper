...
function findPins() {
    ...
    $('.pin-action-row>.action-box').each((index,element)=>{
        // console.log(element)
        const mark =  $(element).attr('mark');

        if(mark && mark == '1') return true;
        ...
        const pinId = element.parentNode.parentNode.getAttribute('data-pin-id');
        const eleId = 'juejin-mark-'+pinId;
        const aid = element.parentNode.parentNode.firstChild.getAttribute('data-author-id');
        ...
        const markObj = {pid:pinId,aid:aid,headImg:headImg}
        $('#'+eleId).on('click',function(e){
            markFunc(e,markObj)
        })
    });
    // if(length >= actionBoxs.length){
    //     return;
    // }
    ...
    //         afterend:在 element 元素的后面。
    //      */
    //     firstEle.insertAdjacentHTML('afterend', markEle)
    //     const markObj = {pid:pinId,aid:aid,headImg:headImg}
    ...
    // let marks = document.getElementsByClassName('plus-mark')
    // for (const ele of marks) {
    //     ele.addEventListener('click',e=>mark(e))
    // }
}
